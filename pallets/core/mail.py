...
from pallets.core.extensions import mail
...
def send_template_message(template=None, ctx=None, *args, **kwargs):
    """
        ...
        Except, it also supports template rendering.
        If you want to use a template then just omit the body and html
        kwargs to Flask-Mail and instead supply a path to a template.
        It will auto-lookup and render text/html messages.

        Example:
            ctx = {'user': current_user, 'reset_token': token}
            send_template_message(
                'Password reset from Foo', ['you@example.com'],
    ...
    if template is not None:
        if "body" in kwargs:
            raise Exception("You cannot have both template and body args.")
        elif "html" in kwargs:
            raise Exception("You cannot have both template and html args.")
...
def _try_renderer_template(template_path, ext="txt", **kwargs):
    """
        Attempt to render a template. We use a try/catch here to avoid
        having to do a path exists based on a relative path to the template.
